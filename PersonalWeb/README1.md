...
- **Dynamic Project Pages**: Click any project to see full details, README, stats, and more
...
├── project-page.js      # Project page JavaScript
└── README.md            # This file
```
...
**In both `script.js` and `project-page.js`:**
```javascript
// In script.js
const CONFIG = {
...
// In project-page.js (line 7)
const githubUsername = urlParams.get('user') || 'yourusername'; // ← Change this too
```
...
- [ ] Learn URL parameters and routing
...
- Understand how project detail pages work (URL parameters)

### How Project Pages Work

When you click on a project card, it navigates to `project.html?project=PROJECT_NAME&user=USERNAME`. The project page:

1. **Reads URL parameters** to get the project name and username
2. **Fetches data from GitHub API**:
   - Repository details (stars, forks, description)
   - README content (converted to HTML)
   - Languages used (with percentages)
   - Recent commits
3. **Dynamically populates** all the content
4. **Fetches related projects** from the same user
5. **Extracts features** from the README automatically

This demonstrates important concepts:
- URL routing without a framework
- Multiple API calls with Promise.all()
- Dynamic content generation
- HTML sanitization (GitHub's API does this for us)
